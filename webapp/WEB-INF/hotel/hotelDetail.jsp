		...
		$('select.selectroom')
				.change(
						function() {
							...
							var price = $('select.selectroom option:selected')
									.val();
...
<body>
	<div class="rightbar">
		<div>
			...
			<div id="hotelname">
				<span class="hoteltype">${hotel.h_type }</span>&nbsp;&nbsp;
				<h1>${hotel.h_name }</h1>
			</div>
			<div id="hoteladdress">
				<p>${hotel.h_address2 }, ${hotel.h_address1 }, ${hotel.h_nation }</p>
			</div>
			<div id="imageArea">
			...
			<div id="hoteloptions">
				<h2>호텔 시설</h2>
				<p class="facilities">
					<span style="font-weight: bold">편의시설&nbsp;</span>${hotel.h_facilities}</p>
				<hr style="margin: 0; width: 95%;">
				<p class="internet">
					<span style="font-weight: bold">인터넷&nbsp;&nbsp;&nbsp;&nbsp;</span>${hotel.h_internet}</p>
				<hr style="margin: 0;">
				<p class="animal">
					<span style="font-weight: bold">반려동물&nbsp;&nbsp;&nbsp;&nbsp;</span>${hotel.h_animal}</p>
				<hr style="margin: 0;">
				<p class="parking">
					<span style="font-weight: bold">주차&nbsp;&nbsp;&nbsp;&nbsp;</span>${hotel.h_parking}</p>
				<hr style="margin: 0;">
				<p class="smoke">
					<span style="font-weight: bold">흡연구역&nbsp;&nbsp;&nbsp;&nbsp;</span>${hotel.h_smoke}</p>
				<hr style="margin: 0;">
				<p class="language">
					<span style="font-weight: bold">지원가능언어&nbsp;&nbsp;&nbsp;&nbsp;</span>${hotel.h_language}</p>
				<hr style="margin: 0;">
			</div>
		</div>
		...
		<div class="register-wrap">
			...
			<table class="register-check">
				<tr>
					...
					<td class="register-check-last"><p>성인 : ${param.adult }</p>
						<p>아이 : ${param.child }</p></td>
			...
			<!--  ****************************************************************************************************************    ******-->
			<form action="reservate.ho" method="post" class="roomreservate"
				name="roomreservate">


				<input type="hidden" name="area" class="area" value="${param.area }">
				<input type="hidden" name="checkin" class="adult"
					value="${param.checkin }"> <input type="hidden"
					name="checkout" class="adult" value="${param.checkout }"> <input
					type="hidden" name="adult" class="adult" value="${param.adult }">
				<input type="hidden" name="child" class="child"
					value="${param.child }"> <input type="hidden" name="room"
					class="room" value="0">
				<table>
					...
					<c:forEach items="${rooms }" var="room" varStatus="r">
						<tr>
							<td class="rtype"><input type="hidden" class="rtypehidden"
								value="${room.r_type }">
							<div onclick="clickRoomTypeinfo()">${room.r_type }</div>
								<ul class="someitems">
							...
							<td class="rprice"><input type="hidden" name="r_price"
								value="${room.r_price }">${room.r_price }</td>
							<td class="rselect"><select class="selectroom">
									<option selected value="0">0
										<c:forEach begin="0" end="${room.r_stock }" varStatus="i">
											...
											<fmt:formatNumber value="${namy}" type="number"
												var="numberType" />

											<option value="${price * namy }">${i.count }&nbsp;(${price * namy })
										</c:forEach>
							</select></td>
							<td class="rtotal"><span class="totalprice${r.count }"></span>
				...
				<input class="rtypename" type="hidden" name="r_type"> <input
					class="totalprice" type="hidden" name="totalprice">
			</form>
		</div>
	</div>
			...
			<form action="Search.ho" method="post" class="leftsearch-form">
				...
				<div class="searchinput">
					...
					<p class="searcharea-p scheduler">
						<i class="material-icons">event</i>&nbsp; <input name="checkin"
							type="text" id="datepicker" placeholder="체크인"
							value="${param.checkin }"><br> <i
							class="material-icons">event</i>&nbsp; <input name="checkout"
					...
					<div id="person" class="searcharea-p">
						&nbsp;<i class="material-icons">face</i><br> 성인:<span
							class="adult">${param.adult }</span>&nbsp;&nbsp; 아동:<span
							class="child">${param.child }</span>&nbsp;&nbsp; 객실:<span
							class="room">${param.room }</span>&nbsp;
					</div>
						...
						<div class="perbox-box">
							<div>
								성인:<input id="minus" class="calc" type="button" value="-"><span
									class="adult">${param.adult }</span><input id="plus"
									class="calc" type="button" value="+">
							</div>
							<div>
								아동:<input id="cminus" class="calc" type="button" value="-"><span
									class="child">${param.child }</span><input id="cplus"
									class="calc" type="button" value="+">
							</div>
							<div>
								객실:<input id="rminus" class="calc" type="button" value="-"><span
									class="room">${param.room }</span><input id="rplus"
									class="calc" type="button" value="+">
							</div>
						</div>
					</div>
					...
					<input type="hidden" name="adult" class="adult"
						value="${param.adult }"> <input type="hidden" name="child"
						class="child" value="${param.child }"> <input
						type="hidden" name="room" class="room" value="${param.room }">
